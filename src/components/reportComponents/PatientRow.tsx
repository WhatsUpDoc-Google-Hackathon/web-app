...
const PatientRow = ({
  ...
  const handleReportClick = (e: React.MouseEvent) => {
    if (onReportSelect) {
      e.preventDefault();
      onReportSelect(patient.latestReport.id);
    }
  };
      ...
      <td className="px-3 md:px-4 lg:px-6 py-3 md:py-4 whitespace-nowrap">
        {onReportSelect ? (
          <button
            ...
            className="px-3 md:px-4 py-2 rounded-lg bg-[var(--color-accent)] text-white font-semibold text-xs md:text-sm shadow hover:bg-primary transition-colors cursor-pointer"
          >
