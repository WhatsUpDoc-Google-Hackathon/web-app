...
export class AudioTranscription {
  ...
  constructor(
    ...
    this.url = url;
    this.apiKey = apiKey;
    this.showVad = showVad;
    this.onTranscription = onTranscription;
    this.onVadPause = onVadPause;
  ...
  async startTranscription(): Promise<void> {
    try {
      ...
      const wsUrl = this.url.endsWith("/api/asr-streaming")
        ? this.url
        : `${this.url.replace(/\/$/, "")}/api/asr-streaming`;
      this.websocket = new WebSocket(
        `${wsUrl}?auth_id=${encodeURIComponent(this.apiKey)}`
      );
      ...
      this.websocket.onclose = (e) => {
        if (e.code !== 1000)
          this.onError(`WebSocket closed unexpectedly: ${e.reason}`);
      };
      ...
      await new Promise<void>((res, rej) => {
        ...
        this.websocket!.onerror = () => (
          clearTimeout(timeout), rej(new Error("WS open failed"))
        );
      });
      ...
      workletNode.port.onmessage = (e) => {
        ...
        const pcm =
          this.audioContext!.sampleRate !== this.SAMPLE_RATE
            ? Array.from(
                {
                  length: Math.floor(
                    (data.length * this.SAMPLE_RATE) /
                ...
                (_, i) =>
                  data[
                    Math.floor(
                      (i * this.audioContext!.sampleRate) / this.SAMPLE_RATE
                    )
                  ] || 0
              )
            : Array.from(data);
